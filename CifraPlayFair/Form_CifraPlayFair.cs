...
{
    public partial class Form_CifraPlayFair : Form
    {
        public Form_CifraPlayFair()
        {
            InitializeComponent();
        }
        ...
        {
			if(txtChave.Text == string.Empty || txtTextoOriginal.Text == string.Empty)
            {
				MessageBox.Show("Favor informar uma chave e um texto válido!");
            }
			else
            {
				string texto = txtTextoOriginal.Text;
				string chave = txtChave.Text;
				string textoCriptografado = Encipher(texto, chave);

				if(txtTextoCriptografado.Text != string.Empty)
                {
					txtTextoCriptografado.Text = string.Empty;
					txtTextoCriptografado.Text = textoCriptografado;
                } 
				else
                {
					txtTextoCriptografado.Text = textoCriptografado;
        ...
        {
			if(txtTextoCriptografado.Text != string.Empty || txtChave.Text != string.Empty)
            {
				string textoCriptografado = txtTextoCriptografado.Text;
				string chave = txtChave.Text;
				string textoDecriptografado = Decipher(textoCriptografado, chave);

				if(txtTextoDecriptografado.Text == string.Empty)
                {
					txtTextoDecriptografado.Text = textoDecriptografado;
                } 
				else
                {
					txtTextoDecriptografado.Text = string.Empty;
					txtTextoDecriptografado.Text = textoDecriptografado;
                }
            }
			else
            {
				MessageBox.Show("Não há texto a ser decriptografado ou a chave não foi informada!");
            }
		}
    }
}